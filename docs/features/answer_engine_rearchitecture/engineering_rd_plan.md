...
- [ ] 增加 `running / long_running / completed / partial_completed / failed`
- [ ] 增加 `5s` 长任务切换
- [ ] 把内部状态文案改成用户语言

完成信号：

- [ ] 前端能区分 `running` 和 `long_running`
- [ ] 默认主路径不再出现内部术语
...
- [ ] 去掉回答阻断
- [ ] 固定默认顺序为 `AnswerPanel -> ReferencesPanel -> ProcessPanel`
- [ ] 放开 pending 中的草稿编辑和会话切换
- [ ] 保持生成中回答可见
- [ ] 去掉 `AnswerPanel` 对 `ReferencesPanel` 的内嵌
- [ ] 去掉 `ProcessPanel` 的思考态自动展开
- [ ] 把 `ProcessOverviewBar` 改成单条用户摘要
