...
- 评估统一走流式 + 严格来源校验 + 引用一致性校验：`python eval/scripts/run_eval_stream.py --testset eval/testsets/testset.json --base-url http://127.0.0.1:9000 --strict-sources --recall-k 5 --concurrency 5 --report eval/reports/report.json`（默认写 `eval/reports/answers.json`）。
